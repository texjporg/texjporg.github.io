...
何も指定しなかった場合，ネットインストーラーは `http://mirror.ctan.org/systems/texlive/tlnet` から各パッケージをダウンロードします。この URL は一番近いミラーサーバーを自動的に判定して接続するものですが，環境やその時点での各サーバーの状況によってはこれではうまくいかない場合があります。起動時に `-select-repository` オプションをつけると，どのミラーサーバーからダウンロードするかを選択することができます。
